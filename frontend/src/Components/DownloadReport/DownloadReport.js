...
function DownloadReport() {
  ...
  const downloadPDF = () => {
    ...
    filteredIncomes.forEach((income, index) => {
      doc.text(
        `${index + 1}. ${income.description} - $${income.amount} on   ${
          income.date
        }`,
        20,
        60 + index * 10
    ...
    filteredExpenses.forEach((expense, index) => {
      doc.text(
        `${index + 1}. ${expense.description} - $${expense.amount} on ${
          expense.date
        }`,
        20,
  ...
  return (
    ...
    <div>
      <div className="filters">
        ...
        <button className="b" onClick={downloadPDF}><span class="material-symbols-outlined">download</span></button>
      </div>
    </div>
    </DownloadReportStyled>
  );
